              ...
              <table className="w-full min-w-[860px] xl:min-w-[980px] table-fixed text-sm">
                ...
                <tbody className="bg-slate-800">
                  {commandeForm.items.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-6 text-slate-500 text-xs">Ajoute les produits commandes pour suivre ce qui doit arriver.</td></tr>
                  ) : (
                    commandeForm.items.map((item, index) => <CommandeLineItem key={index} item={item} produits={produits} onChange={value => updateCommandeItem(index, value)} onRemove={() => removeCommandeItem(index)} />)
                  )}
                </tbody>
                {commandeForm.items.length > 0 && (
                  <tfoot className="bg-slate-750 border-t border-slate-700">
                    <tr>
                      <td colSpan={4} className="py-2 px-3 text-right text-sm font-semibold text-slate-300">Total commande :</td>
